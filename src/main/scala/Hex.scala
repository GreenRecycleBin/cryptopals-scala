...
object Hex:
  ...
  private def xorToBytes(hex: String, b0: Byte): Array[Byte] =
    val bs = toBytes(hex)
  ...
  def xor(hex1: String, hex2: String): String =
    ...
    val (a, b) = if bs1.length < bs2.length then
      (bs2, Array.copyOf(bs1, maxLen))
    else
      (bs1, Array.copyOf(bs2, maxLen))
